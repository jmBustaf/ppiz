<template>
    ...
    <div class="container p-0 w-100">
        <div class="row py-4 m-0 w-100 d-flex justify-content-around">
            <div v-for="(item, index) in this.comics.nombre" v-bind:key="index" class="col-3 comic p-0 my-3 m-0"> 
                <router-link @click="getLink" v-bind:comics="this.comics.detalle[index]" v-bind:to="`/${this.comics.nombre[index]}`"><img class="mx-4 mb-4 w-75 h-75 rounded" v-bind:src="this.comics.image[index]" alt="img-comic"></router-link>
                <p class="mx-4 m-0">nombre: {{this.comics.nombre[index]}}</p>
                <p class="mx-4 m-0">fecha: {{this.comics.fecha[index]}}</p>
            </div>
        </div>
    </div>
